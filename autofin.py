...
async def main():
    ...
    try:
        with open('Token.txt', 'r') as token_file:
            ...
            print(f"Login token: {token[:5]}...")  # Display only first 5 characters of the token
            time.sleep(1)
            print(f"ANJING telah ditemukan, kita sudah terhubung ke server", Fore.GREEN + "mufiiin.com")
    except FileNotFoundError:
        logger.error("Token.txt file not found. Please ensure it exists.")
        exit()
    except Exception as e:
...
if __name__ == '__main__':
    print("\nAlright, we are here! Starting bot...")
    time.sleep(1)
    print("\nBersiap untuk naik ANJING...")
    time.sleep(1)
    print("\nMenyambungkan...")
    time.sleep(1.5)
    try:
